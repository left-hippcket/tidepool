...
function SellerManagement() {
  ...
  const filteredData = sellerGroups.filter(item => {
    ...
    if (searchKeyword.trim()) {
      ...
      // 해당 그룹의 사업자들에서 셀러명과 ticker 검색
      const detail = sellerDetails[item.id];
      const businessMatch = detail?.businesses?.some(business =>
        business.sellerName.toLowerCase().includes(keyword) ||
        business.sellerId.toLowerCase().includes(keyword)
      );

      matchSearch = groupNameMatch || businessMatch;
    }

    return matchManager && matchCategory && matchTerritory && matchRegion && matchStatus && matchSearch;
  });
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-5">
        ...
        <div className="flex flex-wrap gap-3 items-start xl:flex-nowrap">
            <FMSelectSimple
              ...
              onChange={(value) => handleFilterChange(setSelectedManager, value)}
              options={[
                { value: '전체', label: '전체' },
          ...
          <table className="w-full border-collapse">
            <thead className="bg-gray-50">
              <tr className="border-b border-gray-200">
                ...
                <th className="px-4 py-3 text-center text-sm font-medium text-gray-700">은행계좌정보</th>
                <th className="px-4 py-3 text-center text-sm font-medium text-gray-700">메인유통사</th>
                <th className="px-4 py-3 text-left text-sm font-medium text-gray-700">정성평가</th>
            ...
            <tbody>
              {currentItems.map((record) => {
                const qualityEvalDays = getDaysAgo(record.qualityEvaluationDate);
                return (
                  <tr
                    key={record.id}
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-center">
                      {record.hasCompleteBusinessInfo ? (
                    ...
                    <td className="px-4 py-3 text-center">
                      {record.mainDistributors && record.mainDistributors.length > 0 ? (
                        <span className="text-sm text-gray-900">
                          {record.mainDistributors.length === 1
                            ? record.mainDistributors[0]
                            : `${record.mainDistributors[0]} 외`}
                        </span>
                      ) : (
                        <X className="h-5 w-5 text-red-500 mx-auto" />
                      )}
                    ...
                    <td className="px-4 py-3 text-center">
                      {qualityEvalDays !== null ? (
                        <span className="text-sm text-gray-900">{qualityEvalDays}일전</span>
                      ) : (
                        <X className="h-5 w-5 text-red-500 mx-auto" />
                      )}
                    </td>
